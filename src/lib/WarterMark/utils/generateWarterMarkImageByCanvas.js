 ...
 */
function generateWarterMarkImageByCanvas(params = {}) {
  ...
  // 绘制矩形作为背景颜色
  if (!settings.url) {
    throw Error('function generateWarterMarkImageByCanvas url is must!');
  }

  return new Promise(function(resolve, reject) {
    const img = new Image();
    img.onload = function() {
        context.drawImage(img, 0, 0, settings.maskWidth, settings.maskHeight);
        // 移动 canvas 中心
...
export default generateWarterMarkImageByCanvas;
